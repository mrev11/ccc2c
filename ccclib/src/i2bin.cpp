...
void _clp_i2bin(int argno) //signed 16-bit
{
    ...
    stack=base;
    strings((char*)&i,sizeof(c_int16_t)); 
}
...
{
    VALUE *base=stack-argno;
    if( (argno<1) || (base->type!=TYPE_STRING) || (STRINGLEN(base)<sizeof(c_int16_t)) )
    {
        error_arg("bin2i",base,argno);
    }
    c_int16_t i=*(c_int16_t*)(STRINGPTR(base));
    stack=base;
    number(i);
}
...
{
    VALUE *base=stack-argno;
    if( (argno<1) || (base->type!=TYPE_STRING) || (STRINGLEN(base)<sizeof(c_uint16_t)) )
    {
        error_arg("bin2w",base,argno);
    }
    c_uint16_t w=*(c_uint16_t*)(STRINGPTR(base));
    stack=base;
    number(w);
}
...
{
    VALUE *base=stack-argno;
    if( (argno<1) || (base->type!=TYPE_STRING) || (STRINGLEN(base)<sizeof(c_uint32_t)) )
    {
        error_arg("bin2u",base,argno);
    }
    c_uint32_t u=*(c_uint32_t*)(STRINGPTR(base));
    stack=base;
    number(u);
}
...
void _clp_l2bin(int argno) //signed 32-bit
{
    ...
    stack=base;
    strings((char*)&l,sizeof(c_int32_t)); 
}
...
{
    VALUE *base=stack-argno;
    if( (argno<1) || (base->type!=TYPE_STRING) || (STRINGLEN(base)<sizeof(c_int32_t)) )
    {
        error_arg("bin2l",base,argno);
    }
    c_int32_t l=*(c_int32_t*)(STRINGPTR(base));
    stack=base;
    number(l);
}
...
{
    VALUE *base=stack-argno;
    if( (argno<1) || (base->type!=TYPE_STRING) || (STRINGLEN(base)<sizeof(double)) )
    {
        error_arg("bin2f",base,argno);
    }
    double f=*(double*)(STRINGPTR(base));
    stack=base;
    number(f);
}
