...
// fdup(oldfd,inherit)    dup, öröklõdés beállítva
//
// Megjegyzések:
//
...
//    hogy a könyvtárbeli dup alcsonyabb szintû. 
...
//      close(tmpfd);
//
//    ui. az utolsó dup2 mindig örökölhetõvé teszi newfd-t.
//
// 4) Windowson is, Linuxon is a libc-beli dup/dup2 MINDIG 
//    örökölhetõ filé descriptort gyárt (programmal kipróbálva). 
//    Ugyanezért setcloexecflag() továbbra sem implementálható 
//    Windowson. (Ki lehetne erõlködni úgy, hogy elfogyasztunk
//    minden newfd-nél kisebb descriptort.) 
//
// 5) Az stdin öröklõdését így lehet letiltani fdup-pal: 
//
//    tmp=fdup(0); fclose(0); fdup(tmp,.f.); fclose(tmp)
//
//    Ez azon alapszik, hogy fdup mindig az elsõ szabad helyre
...
void _clp_fdup(int argno) 
{
    ...
    //fdup(fd1,.f.)     : dup, az új fd nem öröklõdik
    //fdup(fd1,.f.,.t)  : dup, az új fd nem öröklõdik, a régi lezáródik
    ...
    // Ezt megelõzendõ binárisra állítjuk a defaultot
