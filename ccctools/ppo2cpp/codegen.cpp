...
static int postfixflag=0;       // postfix függvényhívás jelzõ
...
{
    //Kigyûjti a block környezeti változóit.
    //A változók offsetét nem változtatja.
    //Az offset az eredeti függvény stackjére relatív.
    //Az offset-tel az eredeti függvényben címezhetôk a változók.
...
static void blkenvx(parsenode *p)
{
    ...
    //Az offset-tel a blockban címezhetôk a változók.
...
static void blkcode(nodetab *t,int x)
{
    ...
    else if( blkforw )
    {
        ...
        if( blkforw>1 )
        {
            //this:a:...:c(*[2..]) eset 
            p1->right[0]=p3;    
            cgen(p,1); //kiÃ©rtÃ©kelve 'this:a:...' (de :c kihagyva)
    ...
    else  //általános eset
    {
        ...
        blkenvx(lexpr); //összegyûjti lexpr szimbólumait
...
int codegen_statdef_SYMBOL(parsenode *p,void *v)//PROTO 
{
    ...
    //a CCC szinten külsô változót C szinten belsôvé tesszük
    if( sym->cargo & SYM_GLOBSTAT )
    {
...
int codegen_statdef_SYMBOL_ASSIGN_expr(parsenode *p,void *v)//PROTO 
{
    ...
    {
        nltab();fprintf(code,"static stvar _st_%s(%s);",sym->text,expr1->text);
    }
    else if( expr->codegen==codegen_expr_NLSTEXT )
    {
        nltab();fprintf(code,"static stvar _st_%s(nls_text(%s));",sym->text,expr1->text);
    }
...
#define  CLID_EXTERNAL  //clid külsõ static változóban
#define  NEW_CPPLEVEL   //C++ szintû (gyorsított) new

{
    ...
    //most ebbõl kódot kell generálni:
...
int codegen_parexpr_STAR(parsenode *p,void *v)//PROTO
{
    ...
    //függvényhívásból, metódushívásból, vagy array-bõl. 
...
{
    nltab();fprintf(code,"string(%s);",rtxt(p,0));
    return 0;
}
...
{
    nltab();fprintf(code,"string(nls_text(%s));",rtxt(p,0));
    return 0;
}

//---------------------------------------------------------------------------
int codegen_expr_STRINGX(parsenode *p,void *v)//PROTO
{
    nltab();fprintf(code,"stringx(%s);",rtxt(p,0)+1);
    return 0;
}
...
int codegen_expr_LBRACE_PIPE_bargument_PIPE_lexpr_RBRACE(parsenode *p,void *v)//PROTO
{
    ...
    nodetab_blkenv->clean();
    blkenv(p->right[1]); //összegyûjti lexpr szimbólumait
    int i;
...
////outsource_parexpr_STAR_LBRACKET_parexpr0_DOTDOT_parexpr0_RBRACKET
{
    fprintf(src,"*[");
    outsrc(p,0);
    fprintf(src,"..");
...
//---------------------------------------------------------------------------
int outsource_expr_STRINGX(parsenode *p,void *v)//PROTO
{
