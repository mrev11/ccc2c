...
levõ megjelenítõ interfészre. 
A CCC alkalmazásnak nincs szüksége web szerverre,
nem töltõdik le a webes kliens gépére, 
hanem egyszerû programként fut a szolgáltatónál akár a webszerveren, 
...
amik az alkalmazásokhoz rendelt (különbözõ) portokon hallgatóznak.
...
\end{itemize}
de semmi másra, pl. nincs szükség böngészõre.

\begin{quote}\small
A jterminal a Jáva régebbi (1.3.x) változatával is mûködik, 
ekkor azonban külön kell gondoskodni az XML csomag,
az SSL kiegészítés és a Jáva Web Start csomag letöltésérõl és installálásáról. 
Az újabb Jávák elõnye, hogy ezeket az alkatrészeket már 
...
megkönnyíti a Jáva Web Start technológia, amirõl
írok pár sort a következõ pontban.
...
A kliens  továbbiakban a javaws ablakában keletkezõ ikonokkal indíthatja 
az egyszer már letöltött és tárolt Jáva alkalmazásokat.

Ha a szolgáltató Jáva Terminálon át elérhetõ CCC alkalmazásokat
ad közre, akkor minden alkalmazáshoz önálló jnlp filét kell
csinálni. Ezek mind a jterminal.jar-t tartalmazzák letöltendõ
...
\item
    A böngészõt úgy konfiguráljuk, hogy ezt a speciális 
    MIME típust felismerje, és észlelésekor indítsa el
    javaws-t az elõbb leírt módon.
\end{itemize}

Az utóbbi eljárás lehetõvé teszi, hogy a Jáva alkalmazás
letöltését a böngészõbõl indítsuk egy jnlp-re mutató linkre
kattintva.
...
ez magától megtörténik. Ha viszont késõbb máshova rakjuk
...
Az új típus adatait a következõképpen adjuk meg:
