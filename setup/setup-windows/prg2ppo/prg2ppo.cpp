...
//
    ...
    if(!flag()) goto if_5_1;
        ...
        pop();
        line(104);
        string(nls_text("Usage: prg2ppo <input>             //input file (.prg)"));
        _clp_qout(1);
        pop();
        line(105);
        string(nls_text("               [-o<output>]        //output file (.ppo)"));
        _clp_qout(1);
        pop();
        line(106);
        string(nls_text("               [-i<incdir>;]       //include directories"));
        _clp_qout(1);
        pop();
        line(107);
        string(nls_text("               [-u<incfil>;]       //use include file"));
        _clp_qout(1);
        pop();
        line(108);
        string(nls_text("               [-d<sym>[=<val>];]  //define symbol"));
        _clp_qout(1);
        pop();
        line(109);
        string(nls_text("               [@<parfile>]        //parameter file"));
        _clp_qout(1);
        pop();
        line(110);
        _clp_qout(0);
    ...
    pop();
    line(152);
    string(nls_text("Number of define rules    :"));
    _clp_rule_df_get(0);
    _clp_len(1);
    _clp_qout(2);
    pop();
    line(153);
    string(nls_text("Number of translate rules :"));
    _clp_rule_tr_get(0);
    _clp_len(1);
    _clp_qout(2);
    pop();
