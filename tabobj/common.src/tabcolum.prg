...
//STATUS    : közös
//
//function tabColNumber(table,colName)   oszlopnévbõl oszlopsorszám
...
//function tabBlock(table,col,block)     oszlop kiértékelõ blockja
...
local column:=table[TAB_COLUMN], num
    ...
    if(num<=0)
        ...
        taberrDescription("Nemlétezõ oszlopnév")
        taberrArgs(colName)
        tabError(table)
    end
    return num
