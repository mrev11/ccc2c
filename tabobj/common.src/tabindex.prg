...
// A fájl megnyitása elõtt kell hívni, csakis az objektumgeneráló
...
local n, column:=index[IND_COL], cix, col
    ...
    for n:=1 to len(column)
        ...
        if(tabMemoField(table,col))
            ...
            taberrDescription("Memó mezô nem megengedett")
            taberrArgs(col)
            tabError(table) 
        end
...
local aIndex:=table[TAB_INDEX], name
    ...
    if( valtype(ind)=="C" )
        ...
        if( ind==0 )
            ...
            taberrDescription("Nemlétezõ indexnév")
            taberrArgs(name)
            tabError(table) 
        end
    end 
...
// Az oszloplisták (kulcsszegmensek) egyezõsége szerint keres,
...
local iseg, tseg
    ...
    // elõször az ideiglenes indexeket találja meg,
