...
//nem tabAlias()-ból, hanem tabFile()-bõl kell képezni.
...
local sfil:=sdir+dirsep()+f
    ...
    if( !file(lower(sfil)) )
        ...
        //ellenõrizni kell, hogy a directory valóban létrejött-e, 
        //ha a kreálás sikertelen, akkor az valószínûleg
        ...
        if( empty(directory(lower(sdir),"HD")) )
            taberrOperation("tabSlock")
            taberrDescription("Szemafor directory nem hozható létre")
            taberrFilename(sdir)
            tabError(table)
        end
