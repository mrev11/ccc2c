...
//konvertálja a tényleges filét az objektumnak megfelelõen,
...
//    nem enged mezõt kihagyni
//    mezõ típusa nem lehet eltérõ
//    mezõhosszt hajlandó megváltoztatni (csökkent/növel)
//
//    force==.t.-vel erõszakos konverziót csinál
//
//visszatérés:
//    .t., ha OK
...
local logged
    ...
    //a filébõl kiolvasott adatok alapján
    ...
    //az ugyanis összeakadna a már meglévõ,
    //azonos nevû, de másik objektumban (table)
    ...
    //ideiglenes néven létrehozzuk az objektummal megegyezõ 
    //struktúrájú üres filét, elõször töröljük a maradékot
    ...
    for n:=1 to len(colfil)
        ...
        if( 0<i .and. colobj[i][COL_TYPE]==typ )
            ...
            //akkor konvertálható,
            //ha minden mezõ megvan az új táblában,
            //egyezik a típusa, 
            //egyébként jelezni kell, hogy nem konvertálható
            //force==.t. erõszakos konverziót ír elõ
    ...
    while( !tabEof(tabfil) )
        ...
        for n:=1 to len(konvtab)
            value:=eval(konvtab[n][1]) //olvas a régibõl
    ...
    //elõállt a konvertált filé ideiglenes néven, 
