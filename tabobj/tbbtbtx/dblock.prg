...
******************************************************************************
function tabMLock(table,rec,userblock) //többszörös rekord lock
// lockol egy rekordot, a meglevõ lockok megmaradnak
...
*****************************************************************************
function tabRLock(table,userblock) //egyszeres rekord lock
// lockolja az aktuális rekordot, elõzõleg a többi lockot feloldja
...
local len:=_db_read(table[TAB_BTREE],table[TAB_RECBUF],table[TAB_RECPOS])
    if( len!=table[TAB_RECLEN] )
        ...
        taberrDescription("Rekord újraolvasás hibás")
        tabError(table)
    end
    return NIL
...
*****************************************************************************
static function lockDelete(table,record) //lock kivétele locklist-bõl
