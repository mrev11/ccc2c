...
local recno,status,modifkey
    ...
    if( table[TAB_MODIF] )
        ...
        if( tranIsActiveTransaction() ) 
            ...
            if( pup==NIL ) //rekordállapot mentés
                ...
                tranAddToPendingUpdates({;
                    ...
                    ACLONE(table[TAB_MEMODEL])} )  //törlendõ memók
                ...
                if( !keyold==keynew )
                    ...
                    if( !table[TAB_MODIFAPP] )
                        ...
                        if( 0!=status )
                            ...
                            taberrDescription("Kulcs törlése sikertelen")
        ...
        if( status!=0 )
            taberrOperation("tabCommit")
            taberrDescription("Adatrekord kiírása sikertelen")
        ...
        if( modifkey )
            ...
            if( table[TAB_MODIFAPP] )
                ...
                if( status!=0 )
                    taberrOperation("tabCommit")
                    taberrDescription("Kulcs beírása sikertelen")
...
******************************************************************************
function tabMAppend(table,userblock) //append blank, meglévõ lockokat maradnak
...
// key   : az aktuális vezérlõindexnek megfelelõ kulcskifejezés lista
// usrblk: átadja tabAppend()-nek
// flag  : átadja tabAppend()-nek
//
// tabInsert() rááll a kulcslista (key) által azonosított rekordra.
// Ha ilyen rekord nincs, akkor betesz egyet, ennek kulcsmezõit kitõlti 
// a key-ben megadott értékekkel, majd egy újbóli seek-kel ellenõrzi,
// hogy az elsõ seek és a bevitel között egy másik munkaállomás nem
