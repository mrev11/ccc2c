...
function pozitiv(g)
    if(g:varGet()<0)
        alert("Csak pozitív összeg megengedett",{"Javít"})
        return .f.
    end
    return .t.
...
function negativ(g)
    if(g:varGet()>0)
        alert("Csak negatív összeg megengedett",{"Javít"})
        return .f.
    end
    return .t.
...
/*
    ...
    XX-ek hossza egyenlõ text hosszával,
...
************************************************************************
//  Ha sok olyan get mezõ van a maszkban, amelyiknek a 
...
//  adattartalmától, akkor célszerû a getlist-bõl elõre kiszûrni
//  azokat az elemeket, amelyekre úgysem léphet rá a kurzor.
//
//  Ilyenkor a readmodal()-t egyszerûen helyettesítsük readfilt()-tel.
//
//  Ha a get mezõk sorrendjét is kézbe kell venni, akkor ugyanez a
...
//  a kurzor nem léphet rá). Fontos tudni, hogy ez a mûvelet nem 
//  változtatja az eredeti getlist-et, tehát a validokban lévõ
...
//  preBlock tudhassa, hogy nem a read van folyamatban, csak a szûrés,
    ...
    if( !empty(defval) )
        ...
        if( "R"$fix )
            ...
            if( valtype(get:postBlock)!="B".or.eval(get:postBlock) )
                ...
                errobj:description:="Hibás default érték"
                break(errobj)
            end
        end
    end
