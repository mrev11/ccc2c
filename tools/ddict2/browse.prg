...
local dbfname, str1, str2, m, n, col
    ...
    str1:=ctoa(DDICT_DBMSTRUCT)
    str2:=structColumn()

    if( empty(str2) )
        alert(DUMMY:pathname+" nem elérhetõ!")
        return NIL
    end
    ...
    if( alltrim(atoc(str2))==alltrim(DDICT_DBMSTRUCT) )
        alert("A tárolt és a DBF-ban talált struktúra egyezik.")
        return NIL
    end
    ...
    brwMenu(browse,"","Adatszótár és DBF eltérései",{||.f.})
...
local err, browse, index, n
    ...
    if( !structLatest("Csak friss verzió browseolható!") )
        return NIL
    end

    if( !structLoadDummy() )
        alert(DUMMY:pathname+if(file(lower(DUMMY:pathname))," foglalt"," nem található" ))
    else
        brwKillFocus(prevbrw)
        ...
        recover using err
            ...
            if( tabDelIndex(DUMMY:table) )
                alert("Inkompatibilis indexek törölve!")
            else
                alert("Inkompatibilis indexek törölése sikertelen!")
                asize(DUMMY:index,0)
            end
            DUMMY:open
        end sequence
        ...
        brwMenuName(browse,"Tábla: "+DUMMY:pathname)
        brwMenu(browse,"","Sorrend: "+lower(alltrim(DDICT_INDNAME))+" "+index,{||.f.})
...
local crs:=setcursor(0), browse
    ...
    brwArray(browse,str)
    brwColumn(browse,"Mezõ",brwABlock(browse,1),24)
    ...
    brwMenu(browse,"","adatbázis struktúra",{||.f.})
