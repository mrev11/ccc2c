...
static dbf
    if(empty(dbf))
        ...
        tabAddIndex(dbf,{"TABLE","DDICTX",{"TABLE","VERSION","INDFILE"}})
    end
    if(col==NIL)
        return dbf
    end
...
#ifdef _cl_
function _DDICTX_STRUCT(exp)
static col
    if(col==NIL)
        col:=tabColNumber(_DDICTX(),"DBMSTRUCT")
    end
    return _DDICTX(col,exp)
#endif
