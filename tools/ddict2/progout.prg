...
local crlf4:=chr(13)+chr(10)+replicate(" ",4)
  ...
  //  lib  : datadict.rsp   // datadict.lib feltöltéséhez
    ...
    lib:=ddictName()+" &" //1995.09.09 az elsõ obj-ot nem rakta be a lib-be

    DDICT:gotop
    while( !DDICT:eof )

        msg:=message(msg,"Program kimenet: "+DDICT_TABLE)
...
local struct:=ctoa(DDICT_DBMSTRUCT)
...
local memofield:={}, dbmname
    ...
    while( !DDICT:eof .and. alltrim(DDICT_TABLE)==tab .and. ver==DDICT_VERSION )
        ...
        indcol:=wordlist(DDICT_INDFIELD)
        ...
        elseif( len(indcol)<1 )
            ...
            // még ellenõrízni kell, hogy az indexben
            // szereplõ oszlopok léteznek-e
...
*******************************************************************************
static function colExist(struct,col) // ellenõrzi egy mezõ létezését
