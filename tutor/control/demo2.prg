...
local msk,n
    ...
    msk:=mskControlCreate({|g|load(g)},{|g|readmodal(g)},{|g|store(g)})
    mskReplace(msk,30,0) //itt is athelyezheto
    inkey(0)
    mskShow(msk)
    inkey(0)
    mskReplace(msk,1,1) //itt is athelyezheto
    inkey(0)
...
local g2:={}
    ...
    g_lst1:item("3 Öt szép szûzlány õrült írót nyúz")
    g_lst1:item("4 Van, aki forrón szereti!")
    g_lst1:item("5 Próba szerencse")
    g_lst1:item("6 Öt szép szûzlány õrült írót nyúz")
    g_lst1:select(3)
