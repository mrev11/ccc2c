...
// szabályozni az öröklõdést, és ezáltal korrekten kommunikálni egy 
...
local tr,tw 
    ...
    pr:=fdup(p1[1],.f.); fclose(p1[1]) //parent oldali vég (nem öröklõdik)
    pw:=fdup(p2[2],.f.); fclose(p2[2]) //parent oldali vég (nem öröklõdik)
    ...
    tr:=thread_create({||read(pr)}) //ez elõbb lefuthat, mint child elindul
    ...
    // hogy minden filé a megfelelõ helyen le van zárva.
    // A lezárás elmulasztása mûködésképtelenséget okoz:
    //
    ...
    //
    // Ha pw öröklõdését nem tiltjuk le, akkor azt child örökli,
