...
#command SET EXTRA TO <(file)> [<add: ADDITIVE>] [<raw: RAW>]           ;
      => Set( _SET_EXTRAFILE, <(file)>, <.add.>, <.raw.> )
...
#command SET [POSIX] SIGNAL <x:BLOCK,DISABLE,UNBLOCK,ENABLE> =>;
         SetPosixSignal(upper(<(x)>)=="UNBLOCK".or.upper(<(x)>)=="ENABLE")
